...
!! Potential
integer nquant,file_temp
character(len=100) :: file_temperature1, file_temperature2
real*8 g_coup,epsilon
real*8 V_exothermicity,Vc,omg_B,gamma_B,temperature,initial_energy,pop_total_t
...
!! Input/Output
real*8 cnt_frust,cnt_collapse,cnt_init,cnt_term,dtim
real*8,allocatable :: pop(:,:),pop_surf(:,:),pop_amp(:,:),site_en(:)
complex*16,allocatable :: rho(:,:,:)

!! Classical
integer nclass,idistribution,nfile,n_tim,n_instances
real*8,allocatable :: x(:),v(:),acc(:)
real*8,allocatable :: x_old(:),v_old(:),acc_old(:),x_hop(:)
real*8 tim_hop
integer iforward,flag_terminate,flag_frust,flag_reactant,flag_hop,flag_ortho,make_pes
...
!stop
  ...
  allocate(rho(nquant,nquant,i),site_en(i))
...
subroutine files(flag)
  ...
  if(flag==0) then
    ...
    !open(2000,file=file_temperature1)
    !open(40404,file=file_temperature2)
  else
    write(10,*)
...
subroutine init_cond
  ...
  energy0 = initial_energy * wave_to_J
  ...
  do i=1,nclass
    ...
    x(i)=rnd*sig_x - g_coup/(mass(i)*omg(i)**2)    
    call gaussian_random_number(rnd)
    v(i)=(1.d0/mass(i)*(rnd*sig_p))
    
    !sig_x=dsqrt(hbar/(2*mass(1)*omg(i)))
    !sig_p=hbar/(2*sig_x)
    !call gaussian_random_number(rnd)
    !x(i)=rnd*sig_x
    !call gaussian_random_number(rnd)
    !v(i)=(1.d0/mass(i)*(rnd*sig_p)) + sqrt(2*energy0/mass(1))

    !call random_number(rnd)
    !rnd = rnd*2*pi
    !x(i) = dsqrt(2.0*energy0/(mass(i)*omg_B**2)) * dcos(rnd) 
    !v(i) = dsqrt(2*energy0/mass(i)) * dsin(rnd)!(1.d0/mass(i)*(rnd*sig_p))
  enddo
...
subroutine average(i)
  ...
  complex*16 ci_diab(nquant),rho_ad(nquant,nquant),rho_en(nquant,nquant),rho_d(nquant,nquant)
  ...
  if(iaverage==1.and.(mod(i,nstep_avg)==1.or.nstep_avg==1)) then
    ...
    !rho_d=matmul(U,matmul(rho_ad,transpose(U)))
    !rho_en=matmul(rho_d,Hamil_site)
    !site_en(j)=site_en(j)+real(rho_en(1,1)+rho_en(2,2))
    ...
    !  enddo
    !enddo
    !write(5000, *) curr_time*1e15, cdabs(ci_diab)**2
  endif
  !write(5000, *) curr_time*1e15, cdabs(ci_diab)**2
  !call cpu_time(t2)
...
subroutine evolve_classical(dt)
  ...
  !call cpu_time(t1)
  !write(2000, "(5es)") curr_time/1e-15, 0.50 * mass(1) * v**2 /wave_to_J, x*1d10
  if(ifriction==0) then
    !! Step 1
...
subroutine velocity_adjust(state_tentative,ifrust)
  ...
  !write(2000, *) curr_time*1e15, x*1e10, dp
  aa=0.d0
  bb=0.d0
  do i=1,nclass
...
subroutine write_average
  ...
  pop=pop/nf
  !d_ij(1,2,1)=d_ij(1,2,1)/nf
  !vdotd(1,2)=vdotd(1,2)/nf
  !v = v/nf
  rho=rho/nf
  ...
  do i=1,nsteps/nstep_avg
   ...
   ! enddo
   write(100,'(21f15.7)')(i-1)*nstep_avg*dtc*1.d15,real(rho(1,1,i)),real(rho(2,2,i))
  enddo
...
subroutine compute_dij_2state(x_hop,k,kp,dp)
  ...
  !write(4000, *) curr_time*1e15, x*1e10, dp
...
subroutine setup_parameters
  ...
  g_coup=dsqrt(0.5d0*lambda_B*mass(1)*omg_B**2)
  !write(6, *) -g_coup/(mass(1)*omg_B**2)
  !stop

  Hamil_site=0.d0
  Hamil_site(1,1)=V_exothermicity
  Hamil_site(2,2)=0.d0

  Hamil_site(1,2)=Vc; Hamil_site(2,1)=Hamil_site(1,2)

  Hamil_site=Hamil_site*wave_to_J

  omg=omg_B

!dg=Hamil_site(2,2)-Hamil_site(1,1)
!k_marc= 2*pi/hbar * Hamil_site(1,2)**2/sqrt(4*pi*lambda_B*kb*temperature)
!k_marc=k_marc*exp(-(lambda_B-dg)**2/(4*lambda_B*kb*temperature))
!write(6,*) k_marc/1.d12
!stop
...
subroutine compute_potential(H_diab,delV_dels)
  ...
  integer i,j,k1
  real*8 h1,dh1(ncl_site),dx,Amp,a0,b0,Bmin
  ...
  delv_dels(1,1,1)=mass(1)*omg_B**2*x(1)+g_coup
  delv_dels(2,2,1)=mass(1)*omg_B**2*x(1)-g_coup
...
subroutine draw_pes
  ...
  do i=1,8000
    x(1) = x(1) + dx
    call compute_potential(Hamil,delH_dels)
    write(6000,*) x(1)*1.d10, Hamil(1,1)/wave_to_J, Hamil(2,2)/wave_to_J
  enddo
...
subroutine sum_P(x_m, v_m, t_m, energies, k_values, n)
  implicit none
  real*8, intent(in) :: x_m
  real*8, intent(in) :: v_m
  real*8, intent(in) :: t_m
  integer, intent(in) :: n
  real*8 pop_E_t, E, closest_E, k_val
  real*8, intent(in) :: energies(n), k_values(n)
  
  E = 0.5d0 * mass(1) * omg(1)**2 * x_m**2 + 0.5d0 * mass(1) * v_m**2
  E = E/wave_to_J
  call find_closest_k(E, closest_E, k_val, energies, k_values, n)
  pop_E_t = dexp(-k_val*t_m*1d15)
  pop_total_t = pop_total_t + pop_E_t

end subroutine 
!-----------------------------------------------

subroutine find_closest_k(E, closest_E, k_val, energies, k_values, n)
  implicit none
  real*8, intent(in) :: E
  integer, intent(in) :: n
  real*8, intent(out) :: closest_E, k_val
  real*8, intent(in) :: energies(n), k_values(n)
  real*8 :: min_diff
  integer :: i, closest_index

  ! Initialize variables for finding the closest energy
  min_diff = abs(energies(1) - E)
  closest_index = 1

  ! Find the closest energy to E
  do i = 2, n
     if (abs(energies(i) - E) < min_diff) then
        min_diff = abs(energies(i) - E)
        closest_index = i
     end if
  end do

  ! Set the closest energy and corresponding k value
  closest_E = energies(closest_index)
  k_val = k_values(closest_index)

end subroutine find_closest_k
!------------------------------------------------------------------------

subroutine wigner_average
  implicit none
  integer i,j
  character(len=100) :: filename
  real*8 tim, dt
  real*8 :: energies(nfile), k_values(nfile)
  real*8 :: temp_E, temp_k
  integer :: k, file_omg
  integer :: ierr
  
  file_omg = nint(omg(1)/wave_to_J*hbar)
  write(filename, '("E_VS_k_", I0, ".txt")') file_omg !filename = 'E_VS_k_{file_omg}.txt'
   
  ! Open the file
  open(unit=50, file=filename, status='old', action='read')

  ! Read the file contents into arrays
  k = 1
  do while (k <= nfile)
     read(50, *, iostat=ierr) temp_E, temp_k
  ...
  do i = 1, n_tim
     ...
     do j = 1, n_instances
        call init_cond
        call sum_P(x(1), v(1), tim, energies, k_values, nfile)
     end do
     pop_total_t = pop_total_t/n_instances
     write(file_omg, *) tim*1d15, pop_total_t
     tim = tim + dt
  end do

  stop

end subroutine wigner_average
!-------------------------------------------------------------------------
